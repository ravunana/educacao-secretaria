...
import com.ravunana.educacao.secretaria.config.KafkaProperties;
...
class SecretariaKafkaResourceIT {
    ...
    void setup() {
        ...
        SecretariaKafkaResource kafkaResource = new SecretariaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/secretaria-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/secretaria-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
